...
class keyManager extends EventEmitter {
    constructor(cmkID = '', cmkFilePath = __dirname + '/cmk.json', awsRegion = 'us-east-1'){
        super();
        this.dataEncryptionKey = null;
        this.masterKeyID = cmkID;
        ...
        .then(()=>{
            ...
            if (fs.existsSync(this._cmkFilePath)){
                ...
                generateDataKey(this.masterKeyID)
                .then((data)=>{
                ...
                .catch((err)=>{
                    console.log('Error Key Decryption Error or Issue creating new data encryption Key');
                    this.emit('Error', 'Key Decryption Error or Issue creating new data encryption Key', err);
                });
            };
        })
        .catch((err)=>{
...
function getMasterKeyList(){
    var params = {};
	return new Promise((resolve, reject)=>{
		kms.listKeys(params, function(err, data) {
			if (err){
				console.log('Error getting Master Key List')
				console.log(err); // an error occurred
				reject(err);
			} else {
                console.log('Here is an arry of keys attached to this IAM ID:');
                console.dir(data,{depth:null});

				resolve(data.KeyMetadata.KeyId);
				console.log('Success we have a list of Master Keys.')
			};
		});
	});
};
...
function generateDataKey(keyID) {
	console.log('Asking AWS to generate a data encryption key based on key ID: ' + keyID);
    return new Promise((resolve, reject) => {
		const params = {
            KeyId: keyID, 
